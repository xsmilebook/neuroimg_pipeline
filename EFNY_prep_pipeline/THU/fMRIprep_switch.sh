...
wd=/ibmgpfs/cuizaixu_lab/xuhaoshu/code/neuroimg_pipeline/datasets/EFNY/THU/wd/rest
...
#mkdir
if [ ! -d $wd/fmriprep_switch ]; then
mkdir $wd/fmriprep_switch
fi
if [ ! -d $wd/fmriprep_switch/${subj} ]; then
mkdir $wd/fmriprep_switch/${subj}
fi
if [ ! -d $output/fmriprep_switch ]; then
mkdir $output/fmriprep_switch
fi
if [ ! -d $output/fmriprep_switch/${subj} ]; then
mkdir $output/fmriprep_switch/${subj}
fi
...
unset PYTHONPATH; singularity run --cleanenv \
    -B $wd/fmriprep_switch/${subj}:/wd \
    -B $BidsDir:/BIDS \
    -B $output/fmriprep_switch/${subj}:/output \
