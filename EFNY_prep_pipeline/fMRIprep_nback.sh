...
wd=/ibmgpfs/cuizaixu_lab/xuhaoshu/code/neuroimg_pipeline/datasets/EFNY/THU/wd/nback
...
#mkdir
if [ ! -d $wd/fmriprep_nback ]; then
mkdir $wd/fmriprep_nback
fi
if [ ! -d $wd/fmriprep_nback/${subj} ]; then
mkdir $wd/fmriprep_nback/${subj}
fi
if [ ! -d $output/fmriprep_nback ]; then
mkdir $output/fmriprep_nback
fi
if [ ! -d $output/fmriprep_nback/${subj} ]; then
mkdir $output/fmriprep_nback/${subj}
fi
...
unset PYTHONPATH; singularity run --cleanenv \
    -B $wd/fmriprep_nback/${subj}:/wd \
    -B $BidsDir:/BIDS \
    -B $output/fmriprep_nback/${subj}:/output \
