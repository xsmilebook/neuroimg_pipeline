...
def f_headmotion_Jenkinson(fMRIprep_subjDir, taskID, runID, threshold_meanfd, threshold_framefd, frameNum_threshould, threshold_framefdmax):
    ...
    for SN, subj_dir in enumerate(fMRIprep_subjDir):
        if subj_dir.is_dir():
            ...
            # 找到匹配 run 的 confounds 文件
            confounds_files = sorted(subj_dir.glob(f'func/*{taskID}*{runID}*desc-confounds_timeseries.tsv'))
            if not confounds_files:
                continue
            confounds_file = confounds_files[0]  # 取第一个匹配的（通常只有一个）
...
def main():
    parser = argparse.ArgumentParser(description='Compute head-motion (mean FD) report for rest task runs.')
    parser.add_argument(
        '--threshold-meanfd',
        type=float,
        default=0.2,
